...
通过 OreoDeck 启动配置档案时，透明的 status-line proxy 会在每次 API 响应后
捕获 `rate_limits`，并保持原有状态栏输出不变。它不会读取 OAuth 凭据。
`ord status` 和应用会优先使用这个近实时快照，无活动会话时再回退到 Claude
帐户缓存。升级后请为每个配置档案重新打开一次会话，以自动安装 proxy。
